...
bool Mesh::loadFromOBJFile(std::string obj_path, std::vector<TexturedVertex>& out_vertices, std::vector<uint16_t>& out_vertex_indices, vec2& out_size)
{
...
#pragma warning(disable:4996)
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			if (matches == 8) {
				for (int i = 0; i < 3; i++) {
					out_vertices[vertexIndex[i]-1].texcoord = out_uvs[uvIndex[i]-1];
				}
			} else {
